...
it('fails to render without input', () => {
  const { getAllByText } = render(<NotebookCard />);
  expect(getAllByText('')).toHaveLength(3);
});

it('renders2', () => {
    const { getByText, getByLabelText } = render(<NotebookCard parent={state} note={note} notes={notes}/>);
    expect(getByText(String(note.name))).toBeInTheDocument();
    // fireEvent.click(getByLabelText('edit icon'))
  });
