...
it("NotebookCard doesnt render without input", ()=>{
  act(()=>{
    render(<NotebookCard></NotebookCard>, container);
  })
  expect(container.textContent).not.toContain('Notebook')
})

it("NotebookCard render with input", ()=>{
  let state = {
    'state' : {
    pagename: '',
      images:[],
  ...
  let note = state.state.items[0]
  act(()=>{
    render(<NotebookCard parent={state} showModal={(event)=>{}} notes={notes} note={note} />, container);
  })
  expect(container.textContent).toContain(note.name)
})
